#!/usr/bin/env bash
# Builds Tock.app, signs + notarizes + staples it, packages it into a styled
...
#   scripts/release.sh 1.0.0                              # local DMG build with explicit version
#   scripts/release.sh 1.0.0 --publish                    # build + tag + GitHub release
#   scripts/release.sh 1.0.0 --publish --draft
#   scripts/release.sh 1.0.0 --publish --prerelease --notes "Fixes hotkey focus bug"
#   SKIP_NOTARIZE=1 scripts/release.sh                    # unsigned local test build
#
# Credentials are read from .env (or already-exported env vars). See .env.example.
...
Either:
  1. Create .env (see .env.example) with APPLE_ID, APPLE_TEAM_ID, APPLE_APP_PASSWORD
